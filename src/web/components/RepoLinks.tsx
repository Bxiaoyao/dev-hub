...
export function RepoLinks({
  ...
  const repoTooltip =
    info.platform === 'other' ? '打开远程仓库' : `在 ${info.platformLabel} 打开仓库`;

  const compactBtnSize = compactSize === 'sm' ? 'h-8 w-8' : 'h-9 w-9';
  const compactIconSize = compactSize === 'sm' ? 'w-3.5 h-3.5' : 'w-4 h-4';

  const btnClass =
    variant === 'compact'
      ? `${compactBtnSize} shrink-0 flex items-center justify-center bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-600 rounded-lg text-slate-500 dark:text-slate-400 hover:border-blue-400 hover:text-blue-600 dark:hover:text-blue-400 transition-colors disabled:opacity-50`
  ...
  return (
    <div className={`flex items-center gap-1.5 ${className}`}>
      <Tooltip content={repoTooltip}>
        <button type="button" onClick={open(info.repoUrl)} className={btnClass} aria-label={repoTooltip}>
          {variant === 'compact' ? (
            <svg className={compactIconSize} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2">
